...
for e in range(epochs):
    ...
    for x, y in zip(features.values, targets):
        # Forward calculation (output)
        hidden_input = np.dot(features, weights_input_hidden)
        ...
        del_w_hidden_output += hidden_error_term * x[:, None]
    # Update weights
    weights_input_hidden += learnrate * del_w_input_hidden / n_records
    weights_hidden_output += learnrate * del_w_hidden_output / n_records
    #Printing out MSE on the training set
    if e % (epochs / 10) == 0:
...
hidden = sigmoid(np.dot(features, weights_input_hidden))
